    ...
    <section id="skills" className="py-20 px-6 bg-gradient-to-b from-slate-800 to-slate-900 text-white">
      <div className="container mx-auto px-4">
        <div className="max-w-7xl mx-auto">
          ...
          <div className="grid sm:grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-10 items-start">
            {skillCategories.map((category, index) => {
              const IconComponent = category.icon;
              return (
                <div
                  key={index}
                  className="border-2 border-white/20 rounded-xl p-10 min-h-[320px] hover:border-blue-400/50 transition-all duration-300 backdrop-blur-sm bg-white/5 group flex flex-col"
                >
                  <div className="flex items-center gap-4 mb-8">
                    <div className="p-4 bg-blue-600/30 rounded-lg group-hover:bg-blue-600/50 transition-all duration-300">
                  ...
                  <div className="flex flex-wrap gap-3 flex-1">
                    {category.skills.map((skill, skillIndex) => (
                      <span
                        key={skillIndex}
                        className="px-5 py-2.5 bg-white/10 border border-white/20 rounded-full text-base md:text-lg text-gray-200 hover:bg-blue-600/30 hover:border-blue-400/50 transition-all duration-300 font-medium"
                      >
                        {skill}
                      </span>
                    ))}
